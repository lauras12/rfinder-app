...
export default function ListItem(props) {
        ...
        return (
            <li key={props.id}>
                ...
                {(!TokenService.hasAuthToken(config.TOKEN_KEY))? <Link to={'/login'}><button>Create Restaurant review</button></Link> :
                <Link to={`/review/${props.id}`}><button>Create Restaurant review</button></Link>
                }
